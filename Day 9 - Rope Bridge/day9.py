# load data
with open("Day 9 - Rope Bridge/input_sample.txt", encoding="utf-8") as f:
    challenge_input_sample = f.read()
with open("Day 9 - Rope Bridge/input.txt", encoding="utf-8") as f:
...
def part2(challenge_input_variable):
    ...
    for move in challenge_input_variable.splitlines():
        [direction, number] = move.split()
        number = int(number)
        H_location, Knot_location = list(H_locations[-1]), [list(locations[-1]) for locations in Knot_locations]
        if direction == "L":
            for _ in range(number):
                H_location[0] -= 1
                H_locations.append(tuple(H_location))
                Knot_location[0], Knot_locations[0] = move_T(H_location, Knot_location[0], Knot_locations[0])
                for i in range(1,9):
                    Knot_location[i], Knot_locations[i] = move_T(Knot_location[i-1], Knot_location[i], Knot_locations[i])
        elif direction == "R":
            for _ in range(number):
                H_location[0] += 1
                H_locations.append(tuple(H_location))
